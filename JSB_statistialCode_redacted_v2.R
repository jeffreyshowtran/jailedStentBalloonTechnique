...
#one patient has an incorrect birthdate. Input here: 
#patient with MRN 9943059976 has DOB 07-27-1930
data$date_of_birth <- replace(data$date_of_birth, 
                              data$patient_id == 9943059976, 
                              "1930-07-27")

#convert character dates to datetime format 
data$date_of_birth <- as.POSIXct(data$date_of_birth, format="%Y-%m-%d")

#incorrect date of procedure: 
#137/107315229: 
data$date_of_procedure <- replace(data$date_of_procedure, 
                                  data$patient_id == 107315229, 
                                  "2019-06-06")
...
#what is target_lesion_mi??? 
#data[data$target_lesion_mi.factor != data$target_lesion_revasculariz.factor, 
#     c('record_id', 'target_lesion_mi', 'target_lesion_revasculariz')]
data <- data %>% 
...
#question: 4 patients have died, but are NA to whether it was a "cardiac death" 
#or not- should they be included? 
...
#non-cardiac death 
table(data$death_from_any_cause.factor, exclude=FALSE)
data$death_from_any_cause.factor <- ifelse(is.na(data$death_from_any_cause.factor), 
                                           2, 
                                           data$death_from_any_cause.factor)
data$death_from_any_cause.factor = factor(data$death_from_any_cause.factor,levels=c("1","2"))

levels(data$death_from_any_cause.factor)=c("Yes","No")
table(data$death_from_any_cause.factor)

data <- data %>% 
  mutate(ncdeath_1yr = ifelse(death_from_any_cause.factor == 'Yes' & 
                                cardiac_death2.factor == 'No' & 
                                death_dur <= 365, 1, 0)) %>% 
  relocate(ncdeath_1yr, .after=outcome_3yr)
data$ncdeath_1yr <- ifelse(data$cdeath_1yr == 1, 0, data$ncdeath_1yr)
...
data$censor_time2 <- ifelse(is.na(data$censor_time2) & !is.na(data$death_dur),  
                            data$death_dur, 
                            data$censor_time2)
...
paste0("1yr non-cardiac death: ", round(mean(subset(data$ncdeath_1yr, !is.na(data$ncdeath_1yr)))*100, 2), "%")

#understanding patients with multiple outcomes 
df_outcomes <- subset(df, 
                      select = c('record_id', 'patient_id', 'bif_trueLM',
                                 'outcome_1yr', 'TLR_1yr', 'TLR_dur', 
                                 'TLMI_1yr', 'TLMI_dur', 
                                 'cdeath_1yr', 'ncdeath_1yr', 'death_dur'), 
                      df$outcome_1yr == 1)
...
#subset(data$medina_class, data$medina == 999) #two obs are missing medina class
#subset(data$patient_id, data$medina == 999)
...
#paste0("3yr composite: ", round(mean(subset(data$outcome_3yr, !is.na(data$outcome_3yr) & data$bif_trueLM == 1))*100, 2), "%")
paste0("1yr TLR: ", 
...
#paste0("3yr TLR: ", round(mean(subset(data$TLR_3yr, !is.na(data$TLR_3yr) & data$bif_trueLM == 1))*100, 2), "%")
paste0("1yr TLMI: ", 
...
#paste0("3yr TLMI: ", round(mean(subset(data$TLMI_3yr, !is.na(data$TLMI_3yr) & data$bif_trueLM == 1))*100, 2), "%")
...
#paste0("3yr cardiac death: ", round(mean(subset(data$cdeath_3yr, !is.na(data$cdeath_3yr) & data$bif_trueLM == 1))*100, 2), "%")
paste0("1yr non-cardiac death: ", 
       sum(subset(data$ncdeath_1yr, !is.na(data$ncdeath_1yr) & data$bif_trueLM == 1)), " (",
       round(mean(subset(data$ncdeath_1yr, !is.na(data$ncdeath_1yr) & data$bif_trueLM == 1))*100, 2), "%)")
#paste0("3yr cardiac death: ", round(mean(subset(data$cdeath_3yr, !is.na(data$cdeath_3yr) & data$bif_trueLM == 1))*100, 2), "%")
...
#paste0("3yr composite: ", round(mean(subset(data$outcome_3yr, !is.na(data$outcome_3yr) & data$bif_true == 1))*100, 2), "%")
paste0("1yr TLR: ", 
...
#paste0("3yr TLR: ", round(mean(subset(data$TLR_3yr, !is.na(data$TLR_3yr) & data$bif_true == 1))*100, 2), "%")
paste0("1yr TLMI: ", 
...
#paste0("3yr TLMI: ", round(mean(subset(data$TLMI_3yr, !is.na(data$TLMI_3yr) & data$bif_true == 1))*100, 2), "%")
paste0("1yr cardiac death: ", 
...
#aste0("3yr cardiac death: ", round(mean(subset(data$cdeath_3yr, !is.na(data$cdeath_3yr) & data$bif_true == 1))*100, 2), "%")
paste0("1yr non-cardiac death: ", 
       sum(subset(data$ncdeath_1yr, !is.na(data$ncdeath_1yr) & data$bif_true == 1)), " (",
       round(mean(subset(data$ncdeath_1yr, !is.na(data$ncdeath_1yr) & data$bif_true == 1))*100, 2), "%)")
...
####competing risk survival analysis with Fine-Gray subhazard PH model ####

#remove all patients without true bifurcation disease
df <- subset(data, data$bif_true==1)
df2 <- subset(data, data$bif_trueLM==1)

scratch <- subset(data, 
                  select=c('outcome_1yr', 'ncdeath_1yr'))

#drop patients without 1yr outcome data 
df <- subset(df, !is.na(outcome_1yr))


#define event times while also truncating all data to 365d 
#(suspect violation of independent censoring after 365d)
df$etime_main <- ifelse(df$censor_time2 > 365 & df$outcome_1yr != 1, 
                   365, 
                   df$censor_time2)
df$etime_compete <- ifelse(df$ncdeath_1yr == 1, 
                   df$death_dur, 
                   df$etime_main)

scratch <- subset(df, 
                  is.na(etime_compete), 
                  select = c('ncdeath_1yr', 'death_dur', 'etime_main', 
                            'outcome_1yr', 'cdeath_1yr'))

scratch <- subset(df, outcome_1yr == 1 & ncdeath_1yr ==1, 
                  select=c('death_dur', 'etime_main', 'etime_compete',
                           "ncdeath_1yr", 'outcome_1yr'))
#create competing events factor variable 
df$event <- ifelse(df$outcome_1yr == 0 & df$ncdeath_1yr == 0, 0, 
                   ifelse(df$outcome_1yr == 1, 1, 2))
df$event <- factor(df$event, 0:2, labels=c("censor", "composite outcome", "non-cardiac death"))





#create datasets for FG model 
dfmain <- finegray(Surv(etime_main, event) ~ ., data=df, etype="composite outcome")
dfcompete <- finegray(Surv(etime_compete, event) ~ ., data=df, etype="non-cardiac death")


#create dataset containing only patients with LM disease 
#primarily for cumulative incidence plots below 
dfLM <- subset(df, df$bif_LM == 1)
dfLMmain <- finegray(Surv(etime_main, event) ~ ., data=dfLM, etype="composite outcome")
dfLMcompete <- finegray(Surv(etime_compete, event) ~ ., data=dfLM, etype="non-cardiac death")


fit_main <- survfit(Surv(fgstart, fgstop, fgstatus) ~ bif_trueLM, 
                   data=dfmain, weights=fgwt)
fit_compete <- survfit(Surv(fgstart, fgstop, fgstatus) ~ bif_trueLM, 
                    data=dfcompete, weights=fgwt)
...
dfmain$bif_trueLM.factor <- factor(dfmain$bif_trueLM, levels=c(0,1), labels=c("No", "Yes"))

#relabel Syntax score levels 
levels(dfmain$syntax_score.factor) <- c("Low", "Intermediate", "High")

#relevel variables
dfmain$diabetes.factor = relevel(dfmain$diabetes.factor, ref="No")
dfmain$acuity_of_case.factor = relevel(dfmain$acuity_of_case.factor, ref="Elective")
dfmain$mechanical_support.factor = relevel(dfmain$mechanical_support.factor, ref="No")
dfmain$atherectomy_lithotripsy.factor = relevel(dfmain$atherectomy_lithotripsy.factor, ref="No")
dfmain$intervention_for_isr.factor = relevel(dfmain$intervention_for_isr.factor, ref="No")

#label variable names
label(dfmain$age)="Age"
label(dfmain$sex.factor)="Sex"
label(dfmain$diabetes.factor)="Diabetes"
label(dfmain$syntax_score.factor)="Syntax Score"
label(dfmain$acuity_of_case.factor)="Case Acuity"
label(dfmain$bif_trueLM.factor)="LM Bifurcation Disease"
label(dfmain$mechanical_support.factor)="Mechanical Support"
label(dfmain$atherectomy_lithotripsy.factor)="Atherectomy/Lithotripsy"
label(dfmain$intervention_for_isr.factor)="ISR Intervention"
label(dfmain$main_branch_stent_size)="Main branch stent diameter"
label(dfmain$side_branch_stent_diameter)="Side branch stent diameter"



#create cumulative incidence curves 
fg_ci_main <- survfit(Surv(fgstart, fgstop, fgstatus) ~ 1,
                      data=dfmain, weights=fgwt)
fg_ci_mainLM <-survfit(Surv(fgstart, fgstop, fgstatus) ~ 1,
                       data=dfLMmain, weights=fgwt)

#create 2x1 graph format 
par(mfrow = c(1, 2))
#all patients
plot(fg_ci_main, fun = "event", 
...
polygon(c(fg_ci_main$time, rev(fg_ci_main$time)),  # X-coordinates for shading
        c(1 - fg_ci_main$upper, rev(1 - fg_ci_main$lower)),  # Y-coordinates for upper and lower CI
...
plot(fg_ci_mainLM, fun = "event", 
     ...
     conf.int = FALSE,
     yaxt = "n", xaxt = "n", frame.plot = FALSE)  # Suppress default y-axis
polygon(c(fg_ci_mainLM$time, rev(fg_ci_mainLM$time)),  # X-coordinates for shading
        c(1 - fg_ci_mainLM$upper, rev(1 - fg_ci_mainLM$lower)),  # Y-coordinates for upper and lower CI
...
#fit FG model for primary outcome
fgfit_main <- coxph(Surv(fgstart, fgstop, fgstatus) ~ 
                  age + sex.factor + diabetes.factor +  
                  syntax_score.factor + acuity_of_case.factor + 
                  bif_trueLM.factor + mechanical_support.factor + 
...
#fit FG model for primary outcome plus LM/ISR interaction
fgfit_inter <- coxph(Surv(fgstart, fgstop, fgstatus) ~ 
                      age + sex.factor + diabetes.factor +  
                      syntax_score.factor + acuity_of_case.factor + 
                      bif_trueLM.factor + mechanical_support.factor + 
                      atherectomy_lithotripsy.factor + intervention_for_isr.factor + 
                      main_branch_stent_size + side_branch_stent_diameter + 
                      bif_trueLM.factor*intervention_for_isr.factor, 
                    data=dfmain,
                    weights= fgwt)
summary(fgfit_inter)
...
#testing assumptions
#PH test 
ph_test <- cox.zph(fgfit_main) 
ph_test #no red flags 
plot(ph_test) #plots look ok 

#create forest plot 
library(forestmodel)
#dfmain$time <-ifelse(df$censor_time2 > 365 & df$outcome_1yr != 1, 
#                          365, 
#                          df$censor_time2)
#doesn't work for FG models >> print(forest_model(fgfit_main)) 
#doesn't work for FG models >> forest_model(fgfit_main)
...
#fgfit_main$xlevels <- unname(fgfit_main$xlevels)


ggforest(fgfit_inter, 
         main="",
         data=dfmain, 
         fontsize=1)
